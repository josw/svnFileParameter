...
public class StatusHandler implements ISVNStatusHandler, ISVNEventHandler {
    ...
    public StatusHandler(String workspace_path, boolean isRemote) {
        myIsRemote = isRemote;
...
//		if (!pathChangeType.isEmpty() || isLocked || isAddedWithHistory
//				|| isSwitched || !lockLabel.isEmpty()
//				|| !remoteChangeType.isEmpty()) {
		if (!remoteChangeType.trim().isEmpty()) {
			
			String filePath = status.getFile().getPath().replaceFirst(workspacePath, "");
			
			
			Map<String, String> fileStatus = new HashMap<String, String>();
			
			fileStatus.put("pathChangeType", pathChangeType);
			fileStatus.put("propertiesChangeType", propertiesChangeType);
			fileStatus.put("isLocked", (isLocked ? "L" : " "));
			fileStatus.put("isAddedWithHistory", (isAddedWithHistory ? "+" : " "));
			fileStatus.put("isSwitched", (isSwitched ? "S" : " "));
			fileStatus.put("lockLabel", lockLabel);
			fileStatus.put("remoteChangeType", remoteChangeType);
			fileStatus.put("workingRevision", Long.toString(workingRevision));
			fileStatus.put("lastChangedRevision", (lastChangedRevision >= 0 ? String.valueOf(lastChangedRevision) : "?"));
			fileStatus.put("lastChangedAuthor", (status.getAuthor() != null ? status.getAuthor() : "?"));
			fileStatus.put("filePath", filePath);

			result.add(fileStatus);
			
			String line = pathChangeType
    ...
    /*
     * This is an implementation for a
